...
VIDEO_PATH="$1"
SCENE_THRESHOLD=${2:-0.35} # Default to 0.15 if not provided
...
# Use images as a slideshow (5 frames per second)
#ffmpeg -y -nostats -framerate 5 -i "$OUTPUT_DIR/scene_ffmpeg_%03d.png" -c:v libx264 -pix_fmt yuv420p "$SPEEDUP_VIDEO"
...
if (( TS_COUNT < IMG_COUNT )); then
    # Repeat last timestamp (or VIDEO_TOTAL for final image)
...
td, th { border: 1px solid #444; padding: 0.3em; text-align: center; }
...
<table>
<tr><th>Scene Image</th><th>Timestamp (s): Duration (s)</th></tr>
EOF
