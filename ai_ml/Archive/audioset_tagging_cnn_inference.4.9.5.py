...
def sound_event_detection(args):
    ...
    if is_video:
        # Load original video (use video_input_path which might be temp CFR re-encode)
        original_video_clip = VideoFileClip(video_input_path, fps_source='fps')
        target_fps = original_video_clip.fps

        # Ensure overlay clip uses same fps (avoids MoviePy doing weird resampling)
        eventogram_visual_clip = eventogram_visual_clip.set_fps(target_fps)

        # Resize overlay: preserve aspect ratio, height = fraction of video height
        overlay_height = max(1, int(original_video_clip.h * args.overlay_size))
        # compute overlay width preserving aspect ratio of static_eventogram_clip
        # static_eventogram_clip.w might be 0 before first read — use fig_width_px as fallback
        overlay_width = int(round(overlay_height * (static_eventogram_clip.w / max(1, static_eventogram_clip.h)))) \
                        if (static_eventogram_clip.w and static_eventogram_clip.h) else original_video_clip.w

        # but to keep it simple and avoid distortion, make overlay width equal to video width (as you used before)
        overlay_width = original_video_clip.w

        overlay_clip = eventogram_visual_clip.resized((overlay_width, overlay_height))
        overlay_clip = overlay_clip.with_opacity(args.translucency)

        # Composite overlay on top of original, but do NOT attach audio (we'll mux later)
        overlayed_video = CompositeVideoClip([
            original_video_clip,
            overlay_clip.with_position(("right", "bottom"))
        ]).without_audio()

        # Prepare temp files
        tmp_overlay_path = tempfile.NamedTemporaryFile(prefix="evt_overlay_", suffix=".mp4", delete=False).name
        tmp_audio_path = tempfile.NamedTemporaryFile(prefix="evt_audio_", suffix=".m4a", delete=False).name

        try:
            # 1) Render overlay-only video (video encode only). This is the slow step but pure video.
            print(f"🎞  Rendering overlay-only video → {tmp_overlay_path}  @ {target_fps:.3f} fps")
            ...
            # 2) Extract original audio (copy if possible — very fast). Use original source (audio_path)
            print(f"🎧  Extracting original audio (copy if possible) → {tmp_audio_path}")
            # Try copy first (preserves original codec)
            cmd_copy_audio = [
                "ffmpeg", "-y", "-i", video_input_path,
                "-vn", "-acodec", "copy", tmp_audio_path
            ]
            rc = subprocess.run(cmd_copy_audio, capture_output=True, text=True).returncode
            if rc != 0:
                # Fallback: transcode to AAC (reliable)
                print("\033[1;33mAudio copy failed — falling back to re-encode to AAC.\033[0m")
                cmd_reencode_audio = [
                    "ffmpeg", "-y", "-i", video_input_path,
                    "-vn", "-ac", "2", "-ar", "44100", "-c:a", "aac", "-b:a", "128k", tmp_audio_path
                ]
                subprocess.run(cmd_reencode_audio, check=True)

            # 3) Mux overlay-video + original audio without re-encoding video
            print("🔗  Muxing overlay video + audio into final file (no video re-encode)")
            cmd_mux = [
                "ffmpeg", "-y",
                "-i", tmp_overlay_path,
                "-i", tmp_audio_path,
            ...
            mux_proc = subprocess.run(cmd_mux, capture_output=True, text=True)
            if mux_proc.returncode != 0:
                print(f"\033[1;31mError muxing overlay+audio:\033[0m\n{mux_proc.stderr}")
            else:
                print(f"🎹 Saved final video to: {video_path}")

        finally:
            # Clean up temporary files
            for f in (tmp_overlay_path, tmp_audio_path):
                try:
                    if os.path.exists(f):
                        os.remove(f)
                        print(f"🧹 Removed temporary file: {f}")
                except Exception as e:
                    print(f"\033[1;33mWarning: failed to remove temp file {f}: {e}\033[0m")
...
if __name__ == '__main__':
    print(f"Eventogrammer, version 4.9.5, see the original here: https://github.com/qiuqiangkong/audioset_tagging_cnn")
    print(f"")
