...
#. **Docling**: This is the main tool used for processing PDFs and JPGs and generating Markdown output. Make sure you have a docling version post 2.10.0. If something goes wrong, do use the '--force-reinstall' option to make sure that the versions of the dependent modules, eg. easyocr, pytorch, are the right ones. Especially the "Could not initialize NNPACK!" is due to easyocr being used on another machine. Use the pytorch install from Github then and "USE_NNPACK=0 python setup.py install" or add 'export USE_NNPACK=0' to the environment. 
...
sentence_count=$(awk '''BEGIN {RS="[.!?]"; count=0} {count+=NF} END {print count}''' "$in_file")
    ...
    # Output the results
    echo -n "Words: "
    echo "$word_count" | lolcat
    echo -n "Characters: "
    echo "$char_count" | lolcat
    echo -n "Sentences: "
...
#echo "Received arguments: $@" 
    ...
    echo -n "docling"
    echo -n " "
    echo -n "--output $output_dir -v --pdf-backend dlparse_v2 --table-mode accurate --image-export-mode referenced --enrich-picture-classes --ocr-engine tesseract  "
    ...
    docling --output "$output_dir" -v --pdf-backend dlparse_v2 --table-mode accurate --image-export-mode referenced --enrich-picture-classes --ocr-engine tesseract   "$@"
...
# Function to ensure the necessary models are present
ensure_models_are_present() {
    echo "Checking for required models..."


    # Define the specific model directory we need
    local model_dir="$HOME/.cache/docling/models/ds4sd--DocumentFigureClassifier"

    # Check if the specific model directory exists
    if [ ! -d "$model_dir" ]; then
        echo "'ds4sd--DocumentFigureClassifier' not found after general download."
        echo "Attempting to download all"

    # First, try the general download command
    echo "Running 'docling-tools models download'..."
    docling-tools models download

: '        
        # If not, try to download it specifically
        if docling-tools models download-hf-repo ds4sd/DocumentFigureClassifier; then
            echo "Specific model downloaded successfully."
        else
            echo "-------------------------------------------------------------------"
            echo "ERROR: Automatic model download failed."
            echo "Please run the following command manually to download the required model:"
            echo "docling-tools models download-hf-repo ds4sd/DocumentFigureClassifier"
            echo "-------------------------------------------------------------------"
            exit 1
        fi
'
    else
        echo "'ds4sd--DocumentFigureClassifier' model is present."
    fi
}
...
echo 
 echo "OCR via docling, streamlined, version 2.7.1 | author ManamaMa"
...
echo "  --num-threads                 
How many CPU threads it shall use. Do not touch them. Amazingly, the more threads, the worse the performance."
echo "The default added options are: '''--output {the folder of the source file} --pdf-backend dlparse_v2 --table-mode accurate --image-export-mode referenced '''. "
...
if [[ -f "$input_path" ]]; then
    ...
    echo "Using 'pdf2txt' to quick convert to the "$input_path".md  file...";
    pdf2txt.py -o "$input_path".md "$input_path";
    print_word_count "$input_path.md"
...
# Check if it is a directory
    ...
    # Call docling to process the PDF and generate Markdown output in the specified output directory
    time process_input_with_docling "$@"
    md_file="$input_path"/docling_OCR_results_merged.md
