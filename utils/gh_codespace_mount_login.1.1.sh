...
# 10. Initiate an interactive SSH session to the Codes Codespace.
...
echo
        ...
        gh codespace ssh -c "$CSPACE_NAME" -- "nohup rclone serve sshfs $WORKSPACE_PATH --addr :$REMOTE_PORT >/dev/null 2>&1 &"
        if [ $? -ne 0 ]; then
