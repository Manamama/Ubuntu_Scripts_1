...
REMOTE_PORT=2223
#Not below because this is protected by a GitHub code space: 
#REMOTE_PORT=22
        LOCAL_PORT=2222
...
echo " think of using instead: 'gh codespace ports forward <remote-port>:<local-port>... [flags]' : gh codespace ports forward 22:2224 "
echo " but then below: Avoid forwarding port 22; use the script local tunnel (-L 2222:localhost:22 via gh codespace ssh) for SFTP. This proxies the SSH daemon without conflicting with app port forwarding" 
